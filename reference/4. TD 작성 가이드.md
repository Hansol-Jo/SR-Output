...
- 문서 업데이트 시 수정이력 Format을 지켜서 작성할 것. (표 내부는 색상 마킹 대상에서 제외하되 내용은
  동일하게 수정함. Program Source는 원문을 그대로 복사하는 항목이라 이 색상 기반 수정이력 Format을
  적용하지 않음 — 자세한 사항은 아래 'Program Source' 항목 참고)
...
- 소스 안에는 이미 개발자가 SAP에서 직접 작성해 둔 개정이력(Program 헤더 하단의 Modification
  history 표, SEQ가 '01'(최초 생성)·'U01'부터 증가(수정)) 및 실제 변경 로직 표시
  ('Start [Uxx] ... New Logic ... Old Logic ... End [Uxx]' 형태의 ABAP 주석)가 포함되어 있을 수 있음.
  이 주석들은 있는 그대로 유지하고, 새로 추가 · 수정 · 삭제하지 않음 (TD 작성자가 임의로 이력을
  추가하거나 변경 지점을 마킹하는 것이 아님)
- 붙여넣는 순서
