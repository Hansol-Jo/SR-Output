...
- 실행 환경(Windows + MS Word 설치됨)을 활용하여 pywin32(`win32com.client`)로 Word 애플리케이션을 직접 자동화하여 편집할 것
  ...
  - Program Source의 구문 강조 색상(주석 · 키워드 · 일반 코드)은 zdown html에서 추출한 임의의 hex RGB 값이라 정해진 상수가 없으므로 직접 변환해서 `Font.Color`에 지정할 것 — 변환 공식은 `bgr = (blue << 16) | (green << 8) | red` (Word의 `Font.Color`는 0x00BBGGRR 형태의 정수이므로, hex RGB의 R·G·B 순서를 그대로 넣으면 색이 뒤바뀜)
...
- SAP GUI 구문 강조 색상(주석 = 회색, 키워드 = 파란색, 일반 코드 = 검정색)을 Word에도 동일하게
  반영함. zdown html에 이미 각 텍스트 조각의 색상 정보가 포함되어 있으므로, 어떤 단어가 키워드인지
  새로 판단하지 않고 html에 적힌 색상값을 그대로 추출해서 사용함
  - zdown html을 파싱하여 색상이 바뀌는 지점마다 텍스트를 구간으로 나누고, 각 구간의 원본 색상값
    (hex RGB)을 순서대로 추출
  - Word Range를 구간별로 나누어 텍스트를 삽입하고, 각 Range의 `Font.Color`를 추출한 색상값으로
    지정 (RGB → BGR 변환 필요 — 자세한 계산법은 아래 '편집 방법' 참고)
...
- Program Source의 주석 · 키워드 · 일반 코드가 zdown html의 색상값 그대로 구분되어 표시됨  
