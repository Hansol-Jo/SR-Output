...
- 실행 환경(Windows + MS Word 설치됨)을 활용하여 pywin32(`win32com.client`)로 Word 애플리케이션을 직접 자동화하여 편집할 것
  ...
  - Program Source의 구문 색상(주석 · 로직)은 zdown html에서 추출한 임의의 hex RGB 값이라 정해진 상수가 없으므로 직접 변환해서 `Font.Color`에 지정할 것 — 변환 공식은 `bgr = (blue << 16) | (green << 8) | red` (Word의 `Font.Color`는 0x00BBGGRR 형태의 정수이므로, hex RGB의 R·G·B 순서를 그대로 넣으면 색이 뒤바뀜)
...
- zdown html에 표시되는 구문 색상(주석 = 파란색, 로직(코드) = 검정색)을 Word에도 동일하게 반영함.
  어떤 줄이 주석이고 어떤 줄이 로직인지 새로 판단하지 않고, html에 이미 적힌 색상값을 그대로
  추출해서 사용함
...
- Program Source의 주석(파란색) · 로직(검정색)이 zdown html의 색상값 그대로 구분되어 표시됨  
