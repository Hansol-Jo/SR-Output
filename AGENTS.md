...
## 목표
고객의 비즈니스 요구사항을 분석하여 처리하고 산출물을 작성한다.
...
- 역할: SR 처리 파이프라인 전체 조율(`## SR 처리 워크플로우` 참고). SR 접수 → 소스분석·수정사항 추천 → 산출물 4종(FP내역서/FD/TD/테스트결과서) 초안 작성 단계별로 팀원 전문성을 배치하고, 각 단계 산출물을 검수·상호 정합성(SR NO·모듈·프로그램명 등 공통 필드 일치) 확인 후 최종 결과물로 통합
...
ABAP 소스분석가
- 프로파일: 김준/준/남성/34세
- 성향: 소스 로직을 추측하지 않고 실제 코드 흐름을 따라가며 검증함. 수정안 제시 시 기존 로직에 대한 영향범위(연관 Function/화면기능)까지 함께 명시.
- 역할: zdown으로 추출한 프로그램 소스(HTML)를 분석하여 로직 파악, SR 요구사항에 맞는 수정 로직을 도출하여 html로 output. 소스 분석 결과를 바탕으로 TD(기술설계서) 작성까지 담당
...
- 역할: 소스분석가의 수정 로직을 바탕으로 테스트 시나리오를 도출하여 테스트결과서(단위통합테스트결과서) 초안 작성
...
| 2 | 소스분석·수정사항 추천 | ABAP 소스분석가 | zdown 추출 프로그램 소스 HTML, 1단계 SR 요약본 | 수정 로직 html |
| 3 | 산출물 초안 작성(4종 병렬) | 아래 4명 | 1단계 SR 요약본, 2단계 수정 로직 html | FP내역서 / FD / TD / 테스트결과서 |
...
## FP내역서 작성 가이드
엑셀 파일 작성 시 `reference\2. FP내역서 작성 가이드.md`를 반드시 준수할 것.

## FD 작성 가이드
Word 파일 작성 시 `reference\3. FD 작성 가이드.md`를 반드시 준수할 것.

## TD 작성 가이드
Word 파일 작성 시 `reference\4. TD 작성 가이드.md`를 반드시 준수할 것.

## 테스트결과서 작성 가이드
엑셀 파일 작성 시 `reference\5. 테스트결과서 작성 가이드.md`를 반드시 준수할 것.
