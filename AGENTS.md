...
- 역할: SR 처리 파이프라인 전체 조율(`## SR 처리 워크플로우` 참고). SR 접수 → 소스 전후 비교 분석 → 산출물 4종(FP내역서/FD/TD/테스트결과서) 초안 작성 단계별로 팀원 전문성을 배치하고, 각 단계 산출물을 검수·상호 정합성(SR NO·모듈·프로그램ID 공통 필드 일치) 확인 후 최종 결과물로 통합
...
- 역할: ITSM 웹사이트에서 SR 상세정보(요청자·요청일·제목·요청내용 등) 조회, 요구사항을 소스분석가·산출물 작성자가 바로 활용 가능한 구조로 정리하여 전달
...
- 역할: 사용자가 미리 준비해 둔 prep 폴더의 수정 전 · 후 소스 html 파일을 읽어 비교 분석, 어떤 차이점이 있는지 · 비즈니스적으로 어떤 부분이 수정되었는지 파악(`## prep 폴더 취득` 참고). 이 분석 결과를 FP · FD · TD · 테스트결과서 4종 산출물 작성의 공통 근거로 제공하며, TD(기술설계서) 작성까지 직접 담당 — 수정 로직을 직접 개발하거나 추천하지는 않음
...
| 2 | 소스 전후 비교 분석 | ABAP 소스분석가 | prep 폴더의 수정 전 · 후 소스 html, 1단계 SR 요약본 | 비교 분석 결과(차이점 · 비즈니스적 의미) |
...
4단계 검수 시 오케스트레이터는 4개 산출물에 공통으로 존재하는 필드(SR NO·모듈·프로그램ID)가 서로
...
- ITSM 로그인 계정정보 : 'prep\itsm_user.md' 에서 취득
- 서비스운영현황 > 변경/데이터관련요청 > SR 목록에서 검색 가능

## prep 폴더 취득
- 담당: ABAP 소스분석가가 주로 활용 (`## SR 처리 워크플로우` 2단계), FD · TD 작성 시에도 활용
- 사용자는 SR별로 `prep/{SR NO}/` 폴더를 미리 만들어 그 안에 필요한 파일을 넣어 둠 — 에이전트는 SR NO만
  알면(1단계 SR 요약본에 포함됨) 별도 경로 안내 없이 이 폴더를 자동으로 찾음
- `prep/{SR NO}/` 폴더에는 아래 파일이 들어있음
  - zdown 수정 전 · 후 html (필수) : zdown은 SAP GUI 프로그램 소스를 html로 추출하는 사내 도구이며, 사용자가 SR 처리를 요청하기 전에 직접 실행해 준비해 둠 — 에이전트가 zdown을 직접 실행하지 않음
    - 파일명 규칙 : `{프로그램ID}_before` · `{프로그램ID}_after` — `_before`가 수정 전, `_after`가 수정 후 소스임. 두 파일의 소스 구조가 비슷해 내용만으로는 구분이 위험하므로, 반드시 이 파일명으로 전 · 후를 판별하고 내용으로 재판단하지 않음
  - 기존 FD 문서 (선택) : 있으면 FD 버전업 대상으로 사용, 없으면 `template` 폴더 기준으로 신규 작성 (`## 템플릿 파일 취득` 참고)
    - 파일명 규칙 : `reference/fd-guide.md`의 산출물 파일명 규칙과 동일한 접두어
...
## 템플릿 파일 취득
- FP · FD · TD · 테스트결과서 각각의 서식 기준이 되는 참고 파일이 로컬 `template` 폴더에 있음
  (사용자가 미리 준비해 둠 — 에이전트가 새로 만들지 않음). 신규 작성 시(기존 문서가 없는 경우) 이 파일을 복사해서 시작함
- prep 폴더(SR마다 다름, `## prep 폴더 취득` 참고)와는 완전히 다른, 별개의 고정 파일이므로 혼동하지 않음

## 산출물 저장 위치
- 3단계에서 작성하는 산출물 4종(FP내역서 · FD · TD · 테스트결과서)은 모두 `result/{SR NO}/` 폴더에 저장함 — 폴더가 없으면 새로 생성함
- 각 산출물 가이드(`reference/fp-guide.md` 등)의 파일명 규칙에 따른 파일명으로, `result/{SR NO}/` 폴더 바로 아래에 저장함 (하위 폴더 추가 구분 없음)

## 프로그램 식별 정보
FP · FD · TD · 테스트결과서 등 산출물 파일명 · 셀 값에 공통으로 쓰이는 프로그램 관련 placeholder의 의미와 도출 규칙.
- {프로그램ID} : 프로그램 ID / T-code (예: ZSBFMBR0580) 
  — zdown 수정 후 html 파일 중 '{프로그램ID}.html'명으로 된 소스 헤더의 'Program ID' 항목에서 확인
- {프로그램명} : 프로그램 Description (예: [FM] IP 주문실적전표 일괄 취소 프로그램) 
  — zdown 수정 후 html 파일 중 '{프로그램ID}.html'명으로 된 소스 헤더의 'Description' 항목에서 확인
- {모듈명} : {프로그램ID}(Program ID)에서 'Z' 뒤의 알파벳 두 글자로 판별  
  예시 : ZCOBC1180 → 모듈은 'CO'
- {현재유저} : ITSM > 서비스운영현황 > 변경/데이터관련요청 > SR 목록 > SR 검색 시 나오는 '할당자'

## 부서 정보 취득
- opencode에 연결된 atlassian-ktds-kms mcp로 사람 이름을 통해 소속 부서 · 팀을 조회할 수 있음  
  (예: "atlassian-ktds-kms mcp 사용해서 {이름}이 어떤 부서인지 알려줘")
- FD · TD의 '작성팀' 필드({현재유저}가 속한 팀) 조회 시 이 MCP를 활용함
...
- [MED] 프롬프트에 임베딩되는 긴 텍스트는 XML 태그(`<태그명>...</태그명>`) 형식으로 감쌀 것  
  — AI 가 지시문과 입력 데이터를 명확히 구분, 혼용 오류 방지. 출처: 1 회차/프롬프트예제
- [MED] PowerShell 에서 Word Range.Text 읽을 때 \x0007 문자가 섞여 나오면 -replace 로직에서 잘못된 값 표시 가능 — 실제 문서 내용은 정상일 수 있으므로, 생성 후 반드시 사람이 직접 확인 필요. 출처: SRM26062606398 산출물 작성
- [MED] PowerShell 스크립트에서 한글 주석 사용 시 UTF-8 인코딩 깨짐 발생 — 스크립트에서 모든 한글 주석 제거, ASCII 문자만 사용. 출처: SRM26062606398 산출물 작성
